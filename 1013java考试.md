...
- 采用ISO8859-1编码方式时，一个中文字符与一个英文字符一样只占1个字节；采用GB2312或GBK编码方式时，一个中文字符占2个字节；而采用UTF-8编码方式时，一个中文字符会占3个字节。在Java中 在java中，char和byte都是基础数据类型，har类型，是16位，2个字节；java中采用GB2312或GBK编码方式时，一个中文字符占2个字节，而char是2个字节，所以是对的
...
- 线程安全的集合：Vector、Stack、Hashtable
